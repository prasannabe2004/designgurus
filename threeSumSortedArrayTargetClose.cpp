   ...
   public:
    int searchTriplet(vector<int>& arr, int targetSum) {
        ...
        for (int i = 0; i < arr.size() - 2; i++) {
            ...
            while (left < right) {
                // comparing the sum of three numbers to the 'targetSum' can
                // cause overflow so, we will try to find a target difference
                int targetDiff = targetSum - arr[i] - arr[left] - arr[right];
                if (targetDiff ==
                    0) {  //  we've found a triplet with an exact sum
                ...
                if (abs(targetDiff) < abs(smallestDifference) ||
                    ...
                    smallestDifference =
                        targetDiff;  // save the closest difference
                }
